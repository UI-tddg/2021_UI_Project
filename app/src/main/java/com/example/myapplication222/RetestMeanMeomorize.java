...
public class RetestMeanMeomorize extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        korText.setText(RetestSellect.retestarr[current][2]);
        showWord.setText(RetestSellect.retestarr[current][1]);
        showSpeaker.setText(RetestSellect.retestarr[current][3]);
        current++;
    }
    ...
    public void onClick(View view) {
        ...
        if (view == starButton) {
            colorStar.setVisibility(View.VISIBLE);
            update(RetestSellect.retestarr[(current-1)][1], 1);  //중요단어 체크
        }
        if (view == colorStar) {
            colorStar.setVisibility(View.INVISIBLE);
            update(RetestSellect.retestarr[(current-1)][1], 0);  //중요단어 해제
        }
        if(view == nextBtn){
